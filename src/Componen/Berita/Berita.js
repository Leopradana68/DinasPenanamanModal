...
function Berita() {
  ...
  useEffect(() => {
    axios
      .get("http://adminmesuji.embuncode.com/api/news/categories/10")
  ...
  useEffect(() => {
    axios
      .get(
        "http://adminmesuji.embuncode.com/api/news?instansi_id=10&per_page=5&sort_type=desc&sort_by=total_hit"
      )
      .then(function (response) {
  ...
  function gettingData(page, slug, title) {
    ...
    if (slug == null) {
      url =
        "http://adminmesuji.embuncode.com/api/news?instansi_id=10" +
        urlTitle +
        "&per_page=4&page=" +
        page;
    } else {
  ...
  return (
    <Container>
      <br></br>
        ...
        <p>
          {" "}
          <strong> Berita Dinas Penanaman Modal</strong>{" "}
        </p>
                      ...
                      return (
                        <Fragment key={idx}>
                          {NewsCategories === value.slug ? (
                            <ListGroup.Item
                              as="li"
                              onClick={() => handleNewsChange(value.slug)}
